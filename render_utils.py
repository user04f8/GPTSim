...
def calculate_objective(room_positions: Dict[str, Tuple[int, int]], environment: Environment) -> float:
    objective = 0.0
    rooms = list(environment.all_rooms.values())
...
def simulated_annealing(environment: Environment, x_range: Tuple[int, int], y_range: Tuple[int, int], alpha: float, initial_temp: float, cooling_rate: float, connected_optim_dist: int) -> Dict[str, Tuple[int, int]]:
    current_positions = {room.name: (random.randint(*x_range), random.randint(*y_range)) for room in environment.all_rooms.values()}
...
def optimize_room_positions(environment: Environment, x_range: Tuple[int, int], y_range: Tuple[int, int], alpha: float, connected_optim_dist: int) -> Dict[str, Tuple[int, int]]:
    initial_temp = 100.0
    cooling_rate = 0.997
    n_tries = 5
    max_obj = None
    for i in range(n_tries):
        candidate_pos = simulated_annealing(environment, x_range, y_range, alpha, initial_temp, cooling_rate, connected_optim_dist)
